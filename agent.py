...
- בסוף כל תשובה ציין בדיוק מאיזה קובץ הגיע המידע בפורמט:
  📄 מקור: [שם הקובץ המלא] | תיקייה: [שם התיקייה]
- אם מצאת מידע מכמה קבצים, ציין את כולם בנפרד.
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "read_file",
            "description": "קורא את התוכן המלא של קובץ (PDF, Word, Excel, CSV, TXT). חובה להשתמש בזה אחרי שמצאת קובץ רלוונטי — אל תענה בלי לקרוא את התוכן.",
            "parameters": {
                "type": "object",
                "properties": {
